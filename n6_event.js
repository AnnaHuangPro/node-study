...
Events    
        events.EventEmitter    
            ...
            emitter.once(event, listener)//一次性的监听器(监听一次就不再监听了)
            emitter.removeListener(event, listener) //删除指定监听    
            emitter.removeAllListeners([event]) //删除所有监听    
            emitter.setMaxListeners(n) //设置监听数 默认情况下当一个事件的监听超过10个时，EventEmitter 将打印警告信息，0表无限制    
...
http.createServer(function(request, response){   
    ...
    if(request.url!=="/favicon.ico"){
        user = new UserBean();
        user.eventEmit.once('注册成功',function(uname,pwd){
            response.write('注册成功'); 
